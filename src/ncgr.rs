...
impl Ncgr {
    pub fn new<R: Read + Seek>(reader: &mut R) -> io::Result<Self> {
        ...
        let tiles_y = reader.read_u16::<LittleEndian>()?;
        let tiles_x = reader.read_u16::<LittleEndian>()?;

        let depth = reader.read_u32::<LittleEndian>()?;
        let depth = 1 << (depth - 1);
        assert_eq!(depth, 8);

        let mapping = reader.read_u32::<LittleEndian>()?;
        assert_eq!(mapping, 0);
        ...
        assert_eq!(tiles_count as u32, tile_data_size >> 6);
        ...
        let cpos = reader.read_u32::<LittleEndian>()?;
